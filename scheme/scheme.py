...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        return None
    curr, rest = expressions.first, expressions.second
    while rest is not nil:
        scheme_eval(curr, env)
        curr, rest = rest.first, rest.second
    return scheme_eval(curr, env)
    # END PROBLEM 8
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        ops = operands.map(lambda x: scheme_eval(x, env))
        return scheme_apply(self, ops, env)
        # END PROBLEM 5
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    tests, last = expressions, None
    while tests is not nil:
        ev = scheme_eval(tests.first, env)
        if scheme_falsep(ev):
            return ev
        tests, last = tests.second, ev
    return last
    # END PROBLEM 13
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if clause.second is nil:
                return test
            elif isinstance(clause.second, Pair):
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        bond = bindings.first
        check_form(bond, 2, 2)
        symb, vals = Pair(bond.first, symb), Pair(scheme_eval(bond.second.first, env), vals)
        bindings = bindings.second
    check_formals(symb)
    return env.make_child_frame(symb, vals)
    # END PROBLEM 15
